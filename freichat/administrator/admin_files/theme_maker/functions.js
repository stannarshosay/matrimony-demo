...
FreiChat.generate_row = function(name, property, value, state) {
    ...
    return '<tr  id="tr_' + state + 'style_' + property + '"><td class="td_property"><span class="font_style_rules" >' + name + ':</span></td><td>' + FreiChat.get_property_value(property, value, state) + FreiChat.generate_close_btn(property, state) + '</td></tr>';
};
//-------------------------------------------------------------------------------------
 ...
 */
FreiChat.get_property_value = function(property, value, state) {
    ...
    , '#input_' + state + 'style_' + property);

    return '<div contenteditable class="td_value" id="input_' + state + 'style_' + property + '">' + value + '</div>';
...
FreiChat.process_styles = function(id, selected_class, action) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (_styles[i] instanceof Array && _styles[i][0] instanceof Array) {
...
FreiChat.apply_css = function(div, ppty, css_div, oldval, use_passed_id) {
    ...
    var val = $(div).get_value(), class_name, name;
    //console.log(val);
    if (oldval === $.trim(val)) {
        ...
        for (styles in states[key]) {
            ...
            if ($.isArray(states[key][styles])) {
                ...
                for (i = 0; i < len; i++) {
                    ...
                    str += FreiChat.generate_row(name, property, value, 'state_' + id);

                }
            } else {
                value = states[key][styles];
                str_styles += FreiChat.generate_row(name, property, value, 'state_' + id);
                ;
        ...
        for (styles in states[key]) {
            ...
            for (i = 0; i < len; i++) {
                ...
                (function(ppty, id) {
                    ...
                    $('#tr_state_' + id + 'style_' + ppty).mouseover(function() {
                        $('#state_' + id + 'close_style_' + ppty).css('display', 'inline-block');
                    }).mouseout(function() {
                        $('#state_' + id + 'close_style_' + ppty).hide();
                    });

                    $('#input_state_' + id + 'style_' + ppty).bind('textchange', function(event, previousText) {
                        var state = $('#tr_state_' + id + 'style_' + ppty).parent().parent().prev().children(":first").text();
                        var complete_class = FreiChat.selected_class + state;

                        FreiChat.apply_css('#input_state_' + id + 'style_' + ppty, ppty, complete_class, previousText, true);
                    });

                    $('#state_' + id + 'close_style_' + ppty).click(function() {
...
FreiChat.hide_all_elements = function() {
    var parents = ["freichat", "frei_options", "freichat_chatbox", "chatwindow_minimized", "chatwindow_maximized", "freicontain1", "frei_chatroom", "frei_chatroom_lobby", "onfreioffline"];
    ...
    $('#t_chatroom_lobby').click(function() {
        FreiChat.switch_element(['frei_chatroom_lobby']);
    })
