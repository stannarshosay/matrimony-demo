...
FreiChat.upload = function (user, id)
{
    ...
    var data = $jn('#chatboxtextarea' + id).data('data') || {};
    FreiChat.toid = data.id ? data.id.join(",") : id;
    FreiChat.touser = data.user ? data.user.join(",") : user;
    FreiChat.upload_room_id = (data.gid) ? data.gid : FreiChat.in_room
    FreiChat.upload_message_type =(data.messageType) ? data.messageType : 2;
...
FreiChat.getlocal_time = function (GMT_time) {
    ...
    return FreiChat.API.getLocalTime(GMT_time);
};
//-----------------------------------------------------------------------------------------------
FreiChat.getGMT_time = function () {

    return FreiChat.API.getGmtTime();
};
//-----------------------------------------------------------------------------------------------
