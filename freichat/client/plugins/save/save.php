...
class save extends base {
    ...
    public function writeconv($download) {
        ...
        if($this->is_chatroom) {
            ...
            $query = "SELECT * FROM frei_chat WHERE ((frei_chat.\"to\"=? AND frei_chat.\"from\"=?) OR (frei_chat.\"from\"=? AND frei_chat.\"to\"=?))  AND message_type <> 4 AND message_type <> 2 ORDER BY time";            
...
echo "</body></html>";exit; //TODO unreachable code?
        $filename = "$title.html";
