...
//---------------------------------------------------------------------------------------------
    public function savetoDB($filename, $show_name) {
        ...
        if($ext == "jpg" || $ext == "jpen" || $ext == "png" || $ext == "gif" ) {
    
            $message = $this->frei_trans['file_status_uploaded'].": <img src=" . $this->url . "download.php?filename=" . $filename . " />";            
        }else {
        ...
        if ($this->is_chatroom) {
            $message_type = 1;
            $in_room = $this->to;
        } else if((int)$_POST['messagetype'] == 3) {
        
            $message_type = 3;
            $in_room = strip_tags($_POST['roomid']);
        }else{
            $message_type = 0;
            $in_room = -1;
        }
        ...
        foreach($this->to as $to) {
            $this->insert_mesg_query->execute(array($this->frm_id, $this->usr_name, $to, $this->to_name, $message, $this->mysql_now, $time, 
            $message_type, $in_room, $GMT_time));
        }
        
        if($message_type == 3 && !in_array($this->frm_id, $this->to)) {
        //send a copy to myself for group chat
            $this->insert_mesg_query->execute(array($this->frm_id, $this->usr_name, $this->frm_id, $this->to_name, $message, $this->mysql_now, $time, $message_type, $in_room, $GMT_time));
        }
    ...
    <head>
        ...
        <script>
            ...
            if ("<?php echo $upload->uploaded; ?>" == true && "<?php echo $upload->error; ?>" == false)
            {
                ...
                var id = '<?php echo implode($upload->to); ?>';
