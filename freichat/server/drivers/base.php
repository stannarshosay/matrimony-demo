...
//------------------------------------------------------------------------------
    ...
    public function createFreiChatXdb() {
        if (!isset($_SESSION[$this->uid . 'isset_freichatx']) || $_SESSION[$this->uid . 'time'] < $this->time_string - 100) { //To check if session is created or not
			...
			// for getting member gender
				$query1 = "SELECT gender, ".$this->row_username.", plan_status FROM ".$this->usertable." WHERE ".$this->row_userid." ='" . $_SESSION[$this->uid . 'usr_ses_id']."' limit 1";
        		$list_current = $this->db->query($query1)->fetchAll();
				//print_r($list_current);
				$gender = 'Male';
				$plan_status = 'Active';
				$user_name = $_SESSION[$this->uid . 'usr_name'];
				if(isset($list_current) && $list_current !='' && is_array($list_current) && count($list_current) > 0)
				{
					if(isset($list_current[0]['gender']) && $list_current[0]['gender'] !='')
					{
						$gender = $list_current[0]['gender'];
					}
					if(isset($list_current[0]['plan_status']) && $list_current[0]['plan_status'] =='Paid')
					{
						$plan_status = $list_current[0]['plan_status'];
					}
					if(isset($list_current[0][$this->row_username]) && $list_current[0][$this->row_username] !='')
					{
						$user_name = $list_current[0][$this->row_username];
						$user_name = $user_name." ($plan_status)";
					}
				}
				$_SESSION[$this->uid . 'usr_name'] = $user_name;							
				//$_SESSION[$this->uid . 'usr_name'] = $user_name;
				$_SESSION[$this->uid . 'gender'] = $gender;
				
			// for getting member gender
            $query = "INSERT INTO frei_session (username,session_id,time,permanent_id,status,status_mesg,guest,gender)
                      VALUES (:username,:sid,:time,:pid,:status,:status_mesg,:guest,:gender)";
            $stmt = $this->db->prepare($query);

            $stmt->execute(array(
                ":username" => str_replace("amp;", "", $user_name),
...
//------------------------------------------------------------------------------
    public function updateFreiChatXdb($first, $custom_mesg) {
        ...
        if ($_SESSION[$this->uid . 'time'] < $this->online_time || isset($_SESSION[$this->uid . 'usr_name']) == false || $first == 'false' || $this->update_usr_info === true) {
			...
			// for getting member gender
				...
				$_SESSION[$this->uid . 'usr_name'] = $user_name;
			 //To update old session as well as table
