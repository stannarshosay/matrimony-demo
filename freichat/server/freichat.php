...
//for($i=0;$i<=10;$i++){
		...
		$is_user_paid = 'No';
		if(isset($_SESSION[$this->uid . 'usr_name']) && $_SESSION[$this->uid . 'usr_name'] !='' && strpos($_SESSION[$this->uid . 'usr_name'],'Paid') > 0)
		{
			$is_user_paid = 'Yes';
		}
        $user_ids = array();
        foreach ($result as $res) {
			...
			/* code for getting cutome image from tabel to display as user icon */
			
			if(isset($res['session_id']) && $res['session_id'] !='')
			{
			
				$query1 = "SELECT id, gender, photo1, photo1_approve, photo_view_status, photo_protect, photo_password FROM ".$this->usertable." WHERE ".$this->row_userid." ='" . $res[ 'session_id']."' limit 1";				
        		$list_current = $this->db->query($query1);
				if($list_current)
				{
					$temp_avtar = '';
					$main_domain_url = $_SERVER['HTTP_HOST'];
					if($main_domain_url =='localhost' || $main_domain_url =='192.168.1.111')
					{
						if($main_domain_url =='localhost')
						{
							$main_domain_url = 'http://localhost/megamatrymony/';
						}
						else
						{
							$main_domain_url = 'http://192.168.1.111/mega_matrimony/original_script/v2.0/';
						}
					}
					else
					{
						$main_domain_url = $_SERVER['HTTP_HOST'].'/';
						$main_domain_url = str_replace("http://",'',$main_domain_url);
						$main_domain_url = str_replace("https://",'',$main_domain_url);
						if(isset($_SERVER['HTTPS']) && $_SERVER['HTTPS'] =='on')
						{
							$main_domain_url = "https://".$main_domain_url;
						}
						else
						{
							$main_domain_url = "http://".$main_domain_url;
						}
					}
					$main_folder_url = 'assets/';
					$list_current = $list_current->fetch();
					if(isset($list_current) && $list_current !='' && is_array($list_current) > 0)
					{
						if(isset($list_current['gender']) && $list_current['gender'] =='Male')
						{
							$photo_default = 'front_end/img/default-photo/male.png';
							$photo_protected = 'images/photopassword_male.png';
						}
						else if(isset($list_current['gender']) && $list_current['gender'] =='Female')
						{
							$photo_default = 'front_end/img/default-photo/female.png';
							$photo_protected = 'images/photopassword_female.png';
						}
						if(isset($list_current['photo1']) && $list_current['photo1'] !='' && isset($list_current['photo1_approve']) && $list_current['photo1_approve'] =='APPROVED' && isset($list_current['photo_view_status']) && ($list_current['photo_view_status'] == '1' || ($is_user_paid =='Yes' && $list_current['photo_view_status'] == '2')))
						{
							$temp_avtar = 'photos/'.$list_current['photo1'];
						}
						else if(isset($list_current['photo1']) && $list_current['photo1'] !='' && isset($list_current['photo1_approve']) && $list_current['photo1_approve'] =='APPROVED' && isset($list_current['photo_password']) && $list_current['photo_password'] !='' && isset($list_current['photo_protect']) && $list_current['photo_protect'] !='No' && isset($list_current['photo_view_status']) && $list_current['photo_view_status'] == 0)
						{
							$temp_avtar = $photo_protected;
						}
						else
						{
							$temp_avtar = $photo_default;	
						}
					}
					if(isset($temp_avtar) && $temp_avtar != '')
					{
						$avatar_url = $main_domain_url.$main_folder_url.$temp_avtar;
					}
					//print_r($list_current);
				}
			}
			/* code for getting cutome image from tabel to display as user icon */	
			/* commented by mustakim */
