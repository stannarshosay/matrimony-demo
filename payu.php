...
{   
	if ( count( $_POST ) && isset( $_POST['mihpayid'] ) && ! empty( $_POST['mihpayid'] ) ) {
		...
		/*Original condition end*/
		/*$host = (isset( $_SERVER['https'] ) ? 'https://' : 'https://') . $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI'];*/
		
		if ( isset( $_SERVER['REQUEST_URI'] ) && ! empty( $_SERVER['REQUEST_URI'] ) ) $params['surl'] = $params['surl'];
		if ( isset( $_SERVER['REQUEST_URI'] ) && ! empty( $_SERVER['REQUEST_URI'] ) ) $params['furl'] = $params['furl'];
...
class Misc {
	...
	{ 
		if ( $result['status'] === Misc::SUCCESS )
			//echo "<script>window.location = '".$result['data'] ."';<script>";
