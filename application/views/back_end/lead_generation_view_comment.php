...
<div id="view_comment_detail">
	<div class="row">
        <div class="col-lg-5 col-xs-12 col-sm-12 col-md-5">
			...
			<?php if(isset($user_data['phone_no_1']) && $user_data['phone_no_1'] !=''){if(isset($this->common_model->is_demo_mode) && $this->common_model->is_demo_mode == 1){ echo $this->common_model->disable_in_demo_text; } else{ echo $user_data['phone_no_1'];}} else{ echo $dna;} ?>
            </p>
        </div>
    </div>   
	...
	$comment_data = $this->common_model->get_count_data_manual('comments_of_lead_generation',array('index_id'=>$hidd_user_id),2,'*','created_on desc',$page_number,$limit_per_page,0);
	$comment_data_count = $this->common_model->get_count_data_manual('comments_of_lead_generation',array('index_id'=>$hidd_user_id),0,'*','created_on desc ','','',0);
	 ...
	 <div id="view_comment_scroll">
		...
		{?>
            ...
            <div class="pull-right">
                <?php
                    echo $this->common_model->rander_pagination_front('control-panel/lead_generation/view_comment/',$comment_data_count,$limit_per_page);
                ?>
            </div>
        <?php
		}
    ...
    <div class="row form-horizontal">
    	<div class="col-lg-12 col-xs-12 col-sm-12 col-md-12 text-center">
        	<?php
				$access_perm_viewcomm = $this->common_model->check_permission('lead_generation_add_comment');
				...
				if($access_perm_viewcomm !='No')
				{
					...
					{
					?>
					<button type="button" class="btn btn-primary mr10" onclick="lead_generation_add_comment(<?php echo $hidd_user_id;?>)">Add Comment</button>
					<?php
					}
				}
			?>
