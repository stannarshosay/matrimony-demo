...
class Member_model extends CI_Model {
...

	{
		...
		$ele_array = array(
			...
			'cpassword'=>array('type'=>'password','is_required'=>'required'),
		...
		$data_table = array(
			...
			'disp_column_array'=> array('gender','email','mobile','country_name','religion_name','state_name','caste_name','city_name','mtongue_name','birthdate','marital_status','registered_on','assign_to_staff','assign_to_franchise','plan_name','last_login','plan_expired_on')
		...
		$access_perm_cmail = $this->common_model->check_permission('role_name');

		if($access_perm_cmail == 'Full-Access' || $access_perm_cmail='admin')

		{

			$class_name = $this->common_model->class_name;

			if($class_name !='match_making')

			{

				$confirm_mail_btn = array('onClick'=>"return resend_confirm_mail(#id#)",'class'=>'primary','label'=>'Confirm email');

				$this->common_model->button_array[] = $confirm_mail_btn;
		...
		if(isset($personal_where['label_disp']) && ($personal_where['label_disp']=='Match Found' || $personal_where['label_disp']=='Member Match')){
		...
		$this->common_model->common_rander('register', $status, $page, $label_disp, $ele_array, 'id', 0, $other_config);
	...
	{		
		...
		if($this->input->post('fstatus') && $this->input->post('fstatus') !='' && $this->input->post('fstatus') !='All')

		{

			$fstatus = $this->input->post('fstatus');

			$where_search[]= " ( fstatus = '$fstatus' ) ";
...

		{
			...
			$where_search[]= " ( username like ( _latin1 '%$keyword%' ) or mobile like ( _latin1 '%$keyword%' ) or email like ( _latin1 '%$keyword%' ) or matri_id like ( _latin1 '%$keyword%' ) or country_name like ( _latin1 '%$keyword%' ) or state_name like ( _latin1 '%$keyword%' ) or city_name like ( _latin1 '%$keyword%') ) ";
...

	{
...

		{
			...
			$hashed_pass = $this->common_model->cpassword_hash($password);

			$_REQUEST['cpassword'] = $hashed_pass;
		...
		else if(isset($_REQUEST['cpassword']))
		...
		if(isset($_REQUEST['marital_status']) && $_REQUEST['marital_status'] =='Unmarried')

		{

			$_REQUEST['total_children'] ='';
...

		{
			...
			$_REQUEST['staff_assign_id'] = $u_id;

		}

		else if($user_type =='franchise' && $user_type!='')

		{

			$_REQUEST['franchise_assign_id'] = $u_id;
...

	{
...

		{
...

			{
				...
				else if(isset($_REQUEST['mobile_num']) && $_REQUEST['mobile_num'] !='')
...

		{
...

			{
				...
				$this->session->set_flashdata('page_type','physical_detail');
...

			{
				...
				$this->session->set_flashdata('page_type','residence_detail');
...

			{
				...
				$this->session->set_flashdata('page_type','other_info');
...

			{
				...
				$this->session->set_flashdata('page_type','partner_info');
...

			{
				...
				$this->session->set_flashdata('page_type','upload_info');
...

	{
		...
		$this->common_model->update_insert_data_common("comment_master",$data_array_new,array('index_id'=>$hidd_user_id),1,'0');

		if($user_type != 'admin'){

			$this->common_model->update_insert_data_common("register",array('commented'=>'1'),array('id'=>$hidd_user_id),1,'0');
...
// 			),
			...
			array(
				...
				'filed_arr' => array('photo1','photo2','photo3','photo4','photo5'),
				...
				'class_width' =>' col-lg-3 col-md-3 col-sm-6 col-xs-6',

				'not_upload_display'=>'yes',

				'inline_style'=>'style="height:222px;width:190px;object-fit:cover;"',
		...
		$field_main_array = array(				
			...
			array(
				...
				'field_array'=>array(
				    ...
				    'reference'=>array('label'=>'How to know the site'),
				
					'languages_known'=>array('label'=>'Languages Known','type'=>'relation','table_name'=>'mothertongue','disp_column_name'=>'mtongue_name'),
			...
			array(
				...
				'field_array'=>array(
					...
					'cpass_status'=>array('label'=>'Email Verified'),

					'mobile'=>array('label'=>'Mobile','fa_icone'=>'fa fa-phone'),

					'mobile_verify_status'=>array('label'=>'Mobile Verified'),

					'time_to_call'=>array('label'=>'Time To Call'),

					'phone'=>array('label'=>'Phone','fa_icone'=>'fa fa-phone'),
					
					'whatsapp_number'=>array('label'=>'Whatsapp Number','fa_icone'=>'fa fa-whatsapp'),
			...
			array(
				...
				'field_array'=>array(
					...
					'employee_in'=>array('label'=>'Employee Category'),
					...
					'work_address'=>array('label'=>'Work Address'),
			...
			array(
				...
				'field_array'=>array(
					...
					'father_house_name'=>array('label'=>"Father House Name"),
					
					'father_native_place'=>array('label'=>"Father Native Place"),
					
					'father_status'=>array('label'=>'Father Status'),

					'mother_name'=>array('label'=>"Mother's Name"),

					'mother_occupation'=>array('label'=>"Mother's Occupation"),
					
					'mother_house_name'=>array('label'=>"Mother House Name"),
					
					'mother_native_place'=>array('label'=>"Mother Native Place"),
					
					'mother_status'=>array('label'=>'Mother Status'),
					...
					'family_details'=>array('label'=>'About Family/Family Details','is_single'=>'yes','class_width'=>'col-lg-12 col-md-12 col-sm-12 col-xs-12','disp_label'=>'yes'),
			...
			array(
				...
				'field_array'=>array(
					...
					'part_frm_age'=>array('label'=>"Age Preference",'post_filed_concate'=>' to ','post_filed'=>'part_to_age','post_filed_val_after'=>' Years','class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12','label_width'=>'3','val_width'=>9),

					'part_height'=>array('label'=>"Height",'call_back'=>'display_height','post_filed_concate'=>' to ','post_filed'=>'part_height_to','post_filed_call_back'=>'display_height','class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12','label_width'=>'3','val_width'=>9),

					'part_smoke'=>array('label'=>'Smoking','class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12','label_width'=>'3','val_width'=>9),
					...
					'part_mother_tongue'=>array('label'=>'Mother Tongue','type'=>'relation','table_name'=>'mothertongue','disp_column_name'=>'mtongue_name','class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12','label_width'=>'3','val_width'=>9,'disp_label'=>'yes'),
			...
			array(
				...
				'field_array'=>array(
					...
					'part_education'=>array('label'=>'Education','type'=>'relation','table_name'=>'education_detail','disp_column_name'=>'education_name','label_width'=>'3','val_width'=>9,'class_width'=>' col-lg-6 col-md-6 col-sm-12 col-xs-12',),
...

	{
		...
		$age_range_str= '<div class="col-sm-5 col-lg-5 pl0 paddin-mobile">
		...
		$height_range_str= '<div class="col-sm-5 col-lg-5 pl0 paddin-mobile">
		...
		$weight_range_str= '<div class="col-sm-5 col-lg-5 pl0 paddin-mobile">
		...
		$featured_array = $this->common_model->merge_array_all_ddr('featured');
		...
		$ele_array = array(
			...
			'fstatus'=>array('label'=>'Featured Member','display_in'=>'1','type'=>'radio','is_required'=>'required','value_arr'=>$featured_array,'value'=>'All'),
	...
	{	
		...
		if($insert_id != ''){
			...
			$data['error_message'] = 'Confirmation email link send.';
