...
class Search_model extends CI_Model {
	...
	{	
		...
		if($this->input->post('featured') && $this->input->post('featured') !=''){
			if($this->input->post('featured') =='featured'){

				$where_search[]= " ( fstatus = 'Featured' ) ";

				$where_search_filed['fstatus'] = "Featured";
			}
		}
...

		{
			...
			if(isset($photo_search) && trim($photo_search," ") =='photo_search')
...

    {
		...
		$unblockuserid = $this->input->post('blockuserid');
...

	{
		...
		$shortlistuserid = $this->input->post('shortlisteduserid');
				...
				if($sender!='' && $receiver!='' && $message != ''){
...

					{
						...
						if($payments_data_count > 0 && (isset($mem_plan['plan_status']) && $mem_plan['plan_status']=='Paid')){
							...
							$sendNotificationStatus = $this->message_model->send_notification("Your profile has been expressed an interest by ".$sender,$sender,$receiver);
