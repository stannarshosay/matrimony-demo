...
class Search extends CI_Controller {
	...
	function view_profile_app(){
		...
		$current_login_user_id = $this->common_front_model->get_user_id();
		...
		{
			$login_member_data = array();
			$member_member_data = array();
			...
			if(!empty($login_member_data)){
				$user_id = $login_member_data['matri_id'];
			}
			if(!empty($member_member_data)){
...

				{
...

					{
...

						{
							...
							$data1['data']['action'] = $this->common_front_model->common_status_action($user_data);
							$data1['status'] = 'success';
